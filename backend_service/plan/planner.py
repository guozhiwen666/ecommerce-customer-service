...
class TurnPlanner:
    async def predict(self, dialogue_state: DialogueState, *, flow_list: FlowList) -> TurnPlan:
        """
        职责：利用轮次规划器进行路由分析
        Args:
            dialogue_state:
    ...
    def _build_prompt_inputs(self, state: DialogueState, *, flow_list: FlowList) -> dict[str, Any]:
        ...
        available_flows_json = json.dumps({
            "flows": [{k: v for k, v in asdict(flow_object).items() if k != "steps"}
                      for flow_object in flow_list.flows if not flow_object.id.startswith("system")]
        }, ensure_ascii=False)
