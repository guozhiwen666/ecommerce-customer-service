from backend_service.domain.contexts import TaskContext, SystemTaskInterruptedContext, SystemTaskStartedContext, \
    SystemTaskResumedFailedContext, SystemTaskResumedContext, SystemTaskCanceledContext
from backend_service.domain.state import DialogueState
from backend_service.task.commands.command import Command, StartFlowCommand, SetSlotsCommand, ResumedFlowCommand, \
    CancelFlowCommand
...
class CommandProcessor:
    ...
    def process_command(self, commands: list[Command], state: DialogueState, flow_list: FlowList):
        """
        职责：分别处理四种具体的命令
        Args:
            commands:
        ...
        for command in commands:
            if isinstance(command, StartFlowCommand):
                ...
                self._resume_flow(command, state, flow_list)
            elif isinstance(command, CancelFlowCommand):
                self._cancel_flow(state, flow_list)
            else:
                pass

    def _start_flow(self, command: StartFlowCommand, state: DialogueState, flow_list: FlowList):
        """
        ...
        if activate_task is not None:
            ...
            interrupted_flow_name = flow_list.get_flow_by_id(interrupt_flow_id).name
    ...
    def _resume_flow(self, command: ResumedFlowCommand, state: DialogueState, flow_list: FlowList):
        """
        ...
        if activate_task is not None:
            ...
            interrupt_flow_id = activate_task.flow_id
            interrupt_flow_name = flow_list.get_flow_by_id(interrupt_flow_id).name
            ...
            if not resumed:
                ...
                # b) 激活恢复失败的系统流程
                state.start_system_task(SystemTaskResumedFailedContext(
