...
from backend_service.task.flows.links import FlowStepLink, FlowStepStaticLink, FlowStepConditionLink, \
...
class FlowStep:
    ...
    @staticmethod
    def from_dict(step_dict: dict[str, Any]) -> "FlowStep":
        type = step_dict["type"]
        clz = FLOW_STEP_TO_CLASS[type]

        return clz.from_dict(**step_dict)

    @staticmethod
    def load_base_field(step_data: dict[str, Any]) -> dict[str, Any]:
        return {
            "id": step_data["id"],
...
class StartFlowStep(FlowStep):
    ...
    def from_dict(cls, step_dict: dict[str, Any]) -> "StartFlowStep":
        return cls(**FlowStep.load_base_field(step_dict))
...
class ActionFlowStep(FlowStep):
    ...
    def from_dict(cls, step_dict: dict[str, Any]) -> "ActionFlowStep":
        return cls(
            **FlowStep.load_base_field(step_dict),
            action=step_dict["action"],
            args=step_dict.get("arg", {})
        )
